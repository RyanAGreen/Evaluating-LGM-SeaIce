...
# PMIP3
# CCSM4, FGOALS, MRI, MPI, MIROC, IPSL, GISS, CNRM, MMM
...
colorsPMIP3 = ['#ff7f0e','#e377c2','#1f77b4','#bcbd22','#9467bd','#2ca02c','#17becf','orangered','black']

# PMIP4
#UoTCCSM4, CESM1.2, LOVECLIM, AWI, MPI, IPSL, MIROC, MMM
...
colorsPMIP4 = ['#ff7f0e','#8c564b','#F7DC6F','#B8255F','#bcbd22','#2ca02c','#9467bd','black']


# LOVECLIM
# weakNA_AB (LOVE2), weak_AB (LOVE1), MMM
seaiceedge_LOVE = [-58.5, -59.5,-59]
SSTtest_LOVE = [1.61, 2.65, 1.875]
seaicearea_LOVE = [20.27, 15.73, 18.47]
colorsLOVE = ['#F7DC6F','#F7DC6F','black']


# proxy
proxyseaicearea = [11.24]
proxySST = [1.52]
colorsproxy=['darkgrey']
...
PMIP3_zonal_colors = ['#ff7f0e','#e377c2','#1f77b4','#bcbd22','#9467bd','#2ca02c','#17becf','orangered']
...
CNRM_legend = mpatches.Patch(color='orangered', label='CNRM')
...
Proxy_legend = mlines.Line2D([], [], color='black', marker='o', linestyle='None',markersize=8,markerfacecolor='white', label='Proxy data')
...
for i in range(len(seaiceedge_PMIP3)):
    plt.plot(SSTtest_PMIP3[i],seaiceedge_PMIP3[i],marker='^',markersize=8,color=colorsPMIP3[i],zorder=4)
for i in range(len(seaiceedge_PMIP4)):
    plt.plot(SSTtest_PMIP4[i],seaiceedge_PMIP4[i],marker='s',markersize=8,color=colorsPMIP4[i])
# no for loop for love because two different symbols
plt.plot(SSTtest_LOVE[0],seaiceedge_LOVE[0],marker='P',markersize=8,color=colorsLOVE[0])
plt.plot(SSTtest_LOVE[1],seaiceedge_LOVE[1],marker='X',markersize=8,color=colorsLOVE[0])
...
for i in range(len(seaiceedge_PMIP3)):
    plt.plot(SSTtest_PMIP3[i],seaicearea_PMIP3[i],marker='^',markersize=8,color=colorsPMIP3[i],zorder=4)
for i in range(len(seaiceedge_PMIP4)):
    plt.plot(SSTtest_PMIP4[i],seaicearea_PMIP4[i],marker='s',markersize=8,color=colorsPMIP4[i])
# no for loop for love because two different symbols
plt.plot(SSTtest_LOVE[0],seaicearea_LOVE[0],marker='P',markersize=8,color=colorsLOVE[0])
plt.plot(SSTtest_LOVE[1],seaicearea_LOVE[1],marker='X',markersize=8,color=colorsLOVE[0])
# not sure what to do about this symbol
#plt.plot(SSTtest_LOVE[2],seaicearea_LOVE[2],marker='X',markersize=8,color='black')
# proxy
plt.plot(proxySST,proxyseaicearea,marker='o',markersize=8,color='darkgrey')
#linear fit
...
plt.tick_params(bottom=True, top=True, left=True, right=True)
plt.tick_params(axis='both', direction="in", length=7, width=3, color="black")
plt.legend(handles=[PMIP3_legend,PMIP4_legend,weakNA_legend,weakNA_AB_legend,Proxy_legend],ncol=2)
...
plt.tick_params(axis='both', direction="in", length=7, width=3, color="black")
plt.grid()
plt.legend(handles=[CNRM_legend,MIROC_legend,GISS_legend,IPSL_legend,MPI_legend,MRI_legend,FGOALS_legend,AWI_legend,LOVE_legend,CESM_legend,CCSM4_legend,MMM_legend,proxy_legend,LOVE1_legend,LOVE2_legend,degree],ncol=2)
...
plt.show()
# plt.tight_layout()
#plt.savefig('Figures/Figure2all.pdf')
